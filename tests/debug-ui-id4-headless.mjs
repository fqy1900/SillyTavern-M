...
console.log('=== 调查 #ui-id-4 内容 (Headless) ===');
...
try {
    ...
    // 6. 检查 #ui-id-4 的生成来源 - 看看它关联的 tab 链接是什么
    console.log('\n[6] 检查 #ui-id-4 关联的 tab');
    const tabAssociation = await page.evaluate(() => {
        const uiId4 = document.getElementById('ui-id-4');
        if (!uiId4) return { exists: false };
        
        const tabId = uiId4.getAttribute('aria-labelledby');
        const tabLink = tabId ? document.getElementById(tabId) : null;
        
        return {
            exists: true,
            tab_id: tabId,
            tab_link_href: tabLink ? tabLink.getAttribute('href') : '(not found)',
            tab_link_text: tabLink ? tabLink.textContent.substring(0, 50) : '(not found)',
        };
    });

    if (tabAssociation.exists) {
        console.log(`  关联 tab id: ${tabAssociation.tab_id}`);
        console.log(`  关联 tab href: "${tabAssociation.tab_link_href}"`);
        console.log(`  关联 tab text: "${tabAssociation.tab_link_text}"`);
    }

    console.log('\n=== 分析 ===');
    console.log('问题: #ui-id-4 包含了完整的 chat.html 内容');
    console.log('这可能意味着:');
    console.log('1. jQuery UI tabs 识别了某个链接并通过 AJAX 加载了 chat.html');
    console.log('2. 或者原始 chat.html 中有错误的结构导致 tabs 解析异常');
