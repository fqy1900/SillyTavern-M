...
import { router as authPlusRouter } from './endpoints/auth-plus.js';
import { router as chatPlusRouter } from './endpoints/chat-plus.js';
...
// Host login page - 使用自定义登录页
app.get('/login', (req, res) => {
    return res.sendFile('login-plus.html', { root: path.join(serverDirectory, 'public') });
});

// === 新增多用户系统页面路由（无需登录） ===
// 角色卡公开列表页
app.get('/characters', (req, res) => {
    return res.sendFile('characters.html', { root: path.join(serverDirectory, 'public') });
});
// 注册页
app.get('/register', (req, res) => {
    return res.sendFile('register.html', { root: path.join(serverDirectory, 'public') });
});

// === 公开 API（无需登录） ===
// 新用户系统 API（登录/注册/状态）
app.use('/api/auth', authPlusRouter);
// 角色卡公开 API（列表/详情/头像/脚本）
app.use('/api/chat', chatPlusRouter);
...
// Old public API (SillyTavern 原有多用户系统)
...
// 主应用页（三栏布局）
app.get('/app', (req, res) => {
    return res.sendFile('app.html', { root: path.join(serverDirectory, 'public') });
});
// 用户中心
...
app.get('/', cacheBuster.middleware, (request, response) => {
    if (shouldRedirectToLogin(request)) {
        const query = request.url.split('?')[1];
        const redirectUrl = query ? `/characters?${query}` : '/characters';
        return response.redirect(redirectUrl);
    }
    return response.sendFile('app.html', { root: path.join(serverDirectory, 'public') });
});
