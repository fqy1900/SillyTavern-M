...
// 公共 secrets 目录（所有用户共享）
const PUBLIC_SECRETS_DIR = path.join(serverDirectory, 'data', 'default-user');
 ...
 * 使用公共 secrets 目录（default-user），所有用户共享相同的 API 连接
 * @param {import('../users.js').UserDirectoryList} directories User directories (unused, kept for API compatibility)
 * @param {string} key Secret key
 * @param {string?} id Secret ID (optional)
 * @returns {string} Secret value
 */
export function readSecret(directories, key, id = null) {
    // 使用公共目录，所有用户共享 default-user 的 secrets
    const publicDirs = { root: PUBLIC_SECRETS_DIR };
    return new SecretManager(publicDirs).readSecret(key, id);
}

/**
 * Reads the secret state from the secrets file
 * 使用公共 secrets 目录（default-user），所有用户共享相同的 API 连接
 * @param {import('../users.js').UserDirectoryList} directories User directories (unused)
 * @returns {Record<string, boolean>} Secret state
 */
export function readSecretState(directories) {
    const publicDirs = { root: PUBLIC_SECRETS_DIR };
    const state = new SecretManager(publicDirs).getSecretState();
...
export function getAllSecrets(directories) {
    const publicDirs = { root: PUBLIC_SECRETS_DIR };
    const secrets = new SecretManager(publicDirs).getAllSecrets();
...
router.post('/write', (request, response) => {
    try {
        ...
        // 使用公共目录，所有用户共享相同的 API 密钥
        const publicDirs = { root: PUBLIC_SECRETS_DIR };
        const manager = new SecretManager(publicDirs);
...
router.post('/view', (request, response) => {
    try {
        ...
        // 使用公共目录，所有用户共享相同的 API 密钥
        const secrets = getAllSecrets({ root: PUBLIC_SECRETS_DIR });
