...
// 系统预设角色卡目录（只读，所有用户可访问）
// 指向 data/default-user/characters 作为公共角色池
const SYSTEM_CHAR_DIR = path.join(serverDirectory, 'data', 'default-user', 'characters');
...
router.post('/all', async function (request, response) {
    try {
        // 1. 先加载用户自己的角色
        const userFiles = fs.readdirSync(request.user.directories.characters);
        const userPngFiles = userFiles.filter(file => file.endsWith('.png'));
        const userPromises = userPngFiles.map(file => processCharacter(file, request.user.directories, { shallow: useShallowCharacters }));
        const userChars = (await Promise.all(userPromises)).filter(c => c.name);

        // 2. 再加载系统公共角色（标记为系统角色）
        let systemChars = [];
        try {
            if (fs.existsSync(SYSTEM_CHAR_DIR)) {
                const systemFiles = fs.readdirSync(SYSTEM_CHAR_DIR);
                const systemPngFiles = systemFiles.filter(file => file.endsWith('.png'));
                const systemDirectories = { ...request.user.directories, characters: SYSTEM_CHAR_DIR };
                const systemPromises = systemPngFiles.map(file => processCharacter(file, systemDirectories, { shallow: useShallowCharacters }));
                systemChars = (await Promise.all(systemPromises)).filter(c => c.name);
                // 标记为系统角色
                systemChars.forEach(c => c.isSystem = true);
            }
        } catch (e) {
            console.warn('[characters] Failed to load system characters:', e);
        }

        // 3. 合并结果（系统角色在前，用户角色在后）
        return response.send([...systemChars, ...userChars]);
    } catch (err) {
        console.error(err);
