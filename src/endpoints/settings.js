...
// 公共 settings 目录（所有用户共享）
const PUBLIC_SETTINGS_DIR = path.join(serverDirectory, 'data', 'default-user');
...
router.post('/get', (request, response) => {
    let settings;
    try {
        // 使用公共 settings 目录（default-user），所有用户共享相同的 API 连接配置
        const pathToSettings = path.join(PUBLIC_SETTINGS_DIR, SETTINGS_FILE);
