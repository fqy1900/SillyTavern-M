...
// ⚠️ 循环依赖修复：SlashCommandParser.js → power-user.js → SlashCommandParser.js
// 在模块加载时直接 `new SlashCommandParser()` 会触发 TDZ 错误（parser 未初始化）。
// 改为惰性 getter，延迟到第一次访问时才创建实例，此时所有模块已完成加载。
let _parserInstance = null;
function getParser() {
    if (!_parserInstance) {
        _parserInstance = new SlashCommandParser();
    }
    return _parserInstance;
}

export { getParser as parser };

/**
 * @deprecated Use SlashCommandParser.addCommandObject() instead
 */
const registerSlashCommand = (...args) => getParser().addCommand(...args);
...
export function processChatSlashCommands() {
    ...
    for (const [id, inject] of Object.entries(context.chatMetadata.script_injects)) {
        ...
        function reviveFilterClosure() {
            ...
            try {
                return getParser().parse(inject.filter, true);
            } catch (error) {
