...
class PromptManager {
    ...
    init(moduleConfiguration, serviceSettings) {
        ...
        this.handleToggle = (event) => {
            const targetElement = event.target.closest('.' + this.configuration.prefix + 'prompt_manager_prompt');
            if (!targetElement) return;
            const promptID = targetElement.dataset.pmIdentifier;
        ...
        if ('global' === this.configuration.promptOrder.strategy) {
            ...
            const mainElementId = this.updateQuickEdit('main', this.getPromptById('main'));
            if (mainElementId && document.getElementById(mainElementId)) {
                document.getElementById(mainElementId).addEventListener('blur', handleQuickEditSave);
            }

            const nsfwElementId = this.updateQuickEdit('nsfw', this.getPromptById('nsfw'));
            if (nsfwElementId && document.getElementById(nsfwElementId)) {
                document.getElementById(nsfwElementId).addEventListener('blur', handleQuickEditSave);
            }

            const jailbreakElementId = this.updateQuickEdit('jailbreak', this.getPromptById('jailbreak'));
            if (jailbreakElementId && document.getElementById(jailbreakElementId)) {
        ...
        const maxContextEl = document.getElementById('openai_max_context');
        if (maxContextEl) {
            maxContextEl.addEventListener('change', (event) => {
                if (!(event.target instanceof HTMLInputElement)) return;
        ...
        if (maxTokensEl) {
            maxTokensEl.addEventListener('change', (event) => {
                if (this.activeCharacter) this.renderDebounced();
            });
        }
        ...
        const resetBtn = document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_reset');
        if (resetBtn) {
            resetBtn.addEventListener('click', this.handleResetPrompt);
        }
        ...
        const closeButton = document.getElementById(this.configuration.prefix + 'prompt_manager_popup_close_button');
        if (closeButton) {
            closeButton.addEventListener('click', closeAndClearPopup);
        }
        closeAndClearPopup();
        ...
        waitUntilCondition(() => !is_send_press && !is_group_generating, 1024 * 1024, 100).then(async () => {
            try {
                if (true === afterTryGenerate) {
                    ...
                    await this.tryGenerate().finally(async () => {
                        try {
                            ...
                            console.error('PromptManager render afterTry error:', innerErr);
                        }
                    });
                } else {
                    this.profileStart('render');
                ...
                console.error('PromptManager render error:', err);
            }
        }).catch(() => {
    ...
    loadPromptIntoEditForm(prompt) {
        ...
        if (promptField) { promptField.value = prompt.content ?? ''; promptField.disabled = prompt.marker ?? false; }
        if (injectionPositionField) injectionPositionField.value = (prompt.injection_position ?? INJECTION_POSITION.RELATIVE).toString();
        ...
        if (injectionDepthBlock) injectionDepthBlock.style.visibility = prompt.injection_position === INJECTION_POSITION.ABSOLUTE ? 'visible' : 'hidden';
        if (injectionOrderBlock) injectionOrderBlock.style.visibility = prompt.injection_position === INJECTION_POSITION.ABSOLUTE ? 'visible' : 'hidden';
        if (injectionPositionField) injectionPositionField.removeAttribute('disabled');
        if (forbidOverridesField) forbidOverridesField.checked = prompt.forbid_overrides ?? false;
        if (forbidOverridesBlock) forbidOverridesBlock.style.visibility = this.overridablePrompts.includes(prompt.identifier) ? 'visible' : 'hidden';
        if (entrySourceBlock) entrySourceBlock.style.display = isPulledPrompt ? '' : 'none';

        if (isPulledPrompt && entrySource) {
    ...
    handleInjectionPositionChange(event) {
        ...
        if (injectionDepthBlock) injectionDepthBlock.style.visibility = injectionPosition === INJECTION_POSITION.ABSOLUTE ? 'visible' : 'hidden';
        if (injectionOrderBlock) injectionOrderBlock.style.visibility = injectionPosition === INJECTION_POSITION.ABSOLUTE ? 'visible' : 'hidden';
    }
    ...
    clearEditForm() {
        ...
        if (roleField) roleField.selectedIndex = 0;
        if (promptField) { promptField.value = ''; promptField.disabled = false; }
        if (injectionPositionField) { injectionPositionField.selectedIndex = 0; injectionPositionField.removeAttribute('disabled'); }
        ...
        if (injectionTriggerField) injectionTriggerField.value = '';
        if (injectionDepthBlock) injectionDepthBlock.style.visibility = 'unset';
        if (injectionOrderBlock) injectionOrderBlock.style.visibility = 'unset';
        if (forbidOverridesBlock) forbidOverridesBlock.style.visibility = 'unset';
