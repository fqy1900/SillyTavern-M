...
import { characters, chat_metadata, eventSource, event_types, generateQuietPrompt, getCurrentChatId, getRequestHeaders, getThumbnailUrl, saveMetadata, saveSettingsDebounced, this_chid, reinitChatElement, apiFetch } from '../script.js';
...
async function onRenameBackgroundClick(e) {
    ...
    const response = await apiFetch('/api/backgrounds/rename', {
        method: 'POST',
...
export async function getBackgrounds() {
    const response = await apiFetch('/api/backgrounds/all', {
        method: 'POST',
        body: JSON.stringify({}),
    });
    if (response.ok) {
...
async function preloadImageMetadata() {
    try {
        const response = await apiFetch('/api/image-metadata/all', {
            method: 'POST',
...
async function loadFolders() {
    try {
        const response = await apiFetch('/api/backgrounds/folders', {
            method: 'POST',
            body: JSON.stringify({}),
        });
        if (response.ok) {
            ...
            if (thumbnailUpdates.length > 0) {
                await apiFetch('/api/image-metadata/folders/set-thumbnails', {
                    method: 'POST',
...
async function onCreateFolder() {
    ...
    try {
        const response = await apiFetch('/api/image-metadata/folders/create', {
            method: 'POST',
...
async function onRenameFolder(folderId) {
    ...
    try {
        const response = await apiFetch('/api/image-metadata/folders/update', {
            method: 'POST',
...
async function onDeleteFolder(folderId) {
    ...
    try {
        const response = await apiFetch('/api/image-metadata/folders/delete', {
            method: 'POST',
...
async function delBackground(bg) {
    await apiFetch('/api/backgrounds/delete', {
        method: 'POST',
        body: JSON.stringify({
            bg: bg,
        }),
...
export function initBackgrounds() {
    ...
    // 确保 #bg_tabs 中的 tab panel 结构正确（修复重复 #chat 元素的根本原因）
    (function initBackgroundTabsSafe() {
        const $bgTabs = $('#bg_tabs');
        if (!$bgTabs.length) return;
        ...
        // 获取 tab panel 的内容
        const $globalTab = $('#bg_global_tab');
        const $chatTab = $('#bg_chat_tab');
        const globalContent = $globalTab.length ? $globalTab.html() : '';
        const chatContent = $chatTab.length ? $chatTab.html() : '';
        
        // 移除所有 tab panel（包括 ui-id-* 和我们自己的），只保留 ul
        ...
        // 初始化 tabs - 让 jQuery UI 创建它自己的 panel 元素（ui-id-*）
        $bgTabs.tabs();
        
        // 找到 jQuery UI 创建的 panel，并把内容移动到它们中
        const $uiPanels = $bgTabs.find('> [id^="ui-id-"]');
        if ($uiPanels.length >= 2) {
            // 将我们的内容移动到 jQuery UI 创建的 panel 中
            $uiPanels.eq(0).html(globalContent);
            $uiPanels.eq(1).html(chatContent);
            
            // 更新 ID 为我们期望的名字（这样其他代码可以正常引用）
            $uiPanels.eq(0).attr('id', 'bg_global_tab');
            $uiPanels.eq(1).attr('id', 'bg_chat_tab');
            
            // 同步更新 tab 按钮的 href 和 aria-controls
            const $lis = $bgTabs.find('ul li');
            $lis.eq(0).find('a').attr('href', '#bg_global_tab');
            $lis.eq(1).find('a').attr('href', '#bg_chat_tab');
            $lis.eq(0).attr('aria-controls', 'bg_global_tab');
            $lis.eq(1).attr('aria-controls', 'bg_chat_tab');
        }
    })();
    
    // 清理重复的 #chat、#sheld、#bg_tabs 元素
    // 这些元素是由于 jQuery UI tabs 在初始化时通过 AJAX 错误地加载了整个 chat.html 导致的
    (function removeDuplicateElements() {
        // 找到正确的主元素
        const $body = $('body');
        const $correctSheld = $body.children('#sheld').first();
        const $correctChat = $correctSheld.find('> #chat');
        const $correctBgTabs = $('#Backgrounds').find('#bg_tabs');
        
        // 删除所有嵌套在其他元素中的重复 #chat
        document.querySelectorAll('#chat').forEach(function (el) {
            // 如果 #chat 的父元素不是 #sheld，或者 #sheld 不是 body 的直接子元素，则删除
            const parent = el.parentElement;
            const grandParent = parent?.parentElement;
            if (!parent || parent.id !== 'sheld' || grandParent !== document.body) {
                el.remove();
            }
        });
        
        // 删除所有嵌套在其他元素中的重复 #sheld
        document.querySelectorAll('#sheld').forEach(function (el) {
            // 如果 #sheld 的父元素不是 body，则删除
            if (el.parentElement !== document.body) {
                el.remove();
            }
        });
        
        // 删除所有嵌套在其他 #bg_tabs 中的重复 #bg_tabs（递归嵌套）
        document.querySelectorAll('#bg_tabs').forEach(function (el) {
            // 如果 #bg_tabs 的父元素不是在正确的位置，则删除
            const parent = el.parentElement;
            const parentId = parent?.id || '';
            const parentClass = parent?.className || '';
            
            // 正确的位置应该是在 #Backgrounds drawer 中，并且不在其他 #bg_tabs 内部
            let insideOtherBgTabs = false;
            let node = parent;
            while (node && node.nodeType === 1) {
                if (node.id === 'bg_tabs') {
                    insideOtherBgTabs = true;
                    break;
                }
                node = node.parentElement;
            }
            
            if (insideOtherBgTabs) {
                el.remove();
            }
        });
        
        // 额外清理：删除所有 ui-id-* 中包含完整 HTML 结构的元素
        document.querySelectorAll('[id^="ui-id-"]').forEach(function (el) {
            // 如果这个元素包含 #chat 或 #sheld，则删除它
            if (el.querySelector('#chat') || el.querySelector('#sheld')) {
                el.remove();
            }
        });
        
        // 清理后重新初始化 chatElement，确保它指向正确的 DOM 元素
        reinitChatElement();
    })();
