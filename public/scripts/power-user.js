...
// ⚠️ 循环依赖修复：SlashCommandParser/SlashCommand/SlashCommandArgument 移除顶层 static import，
// 改为在 jQuery DOM ready 回调内动态 import。
// PARSER_FLAG 仅在 jQuery ready 回调中使用，也改为动态 import（回调执行时循环链已解除）。
const _scLazy = { SlashCommandParser: null, SlashCommand: null, SlashCommandArgument: null, SlashCommandNamedArgument: null, ARGUMENT_TYPE: null, PARSER_FLAG: null };
async function loadSlashCommandClasses() {
    if (_scLazy.SlashCommandParser) return _scLazy;
    const [scParserMod, scMod, scArgMod] = await Promise.all([
    ...
    _scLazy.SlashCommandParser = scParserMod.SlashCommandParser;
    _scLazy.SlashCommand = scMod.SlashCommand;
    _scLazy.SlashCommandArgument = scArgMod.SlashCommandArgument;
    _scLazy.SlashCommandNamedArgument = scArgMod.SlashCommandNamedArgument;
    _scLazy.ARGUMENT_TYPE = scArgMod.ARGUMENT_TYPE;
    _scLazy.PARSER_FLAG = scParserMod.PARSER_FLAG;
    return _scLazy;
}
// 顶层占位：SlashCommandParser 等在 jQuery ready 中动态 import（var 可在同一作用域重复声明）
var SlashCommandParser = /** @type {any} */ (null);
var SlashCommand = /** @type {any} */ (null);
var SlashCommandArgument = /** @type {any} */ (null);
var SlashCommandNamedArgument = /** @type {any} */ (null);
var ARGUMENT_TYPE = /** @type {any} */ (null);
var PARSER_FLAG = /** @type {any} */ (null);
...
jQuery(async () => {
    // ⚠️ 循环依赖修复：等待 slash-command 类加载完成后再执行
    // 此时所有模块已完成初始化，static import 不再触发 TDZ
    await loadSlashCommandClasses();
    // 赋值给顶层 var 变量（var 在模块顶部 hoisted，无 TDZ）
    SlashCommandParser = _scLazy.SlashCommandParser;
    SlashCommand = _scLazy.SlashCommand;
    SlashCommandArgument = _scLazy.SlashCommandArgument;
    SlashCommandNamedArgument = _scLazy.SlashCommandNamedArgument;
    ARGUMENT_TYPE = _scLazy.ARGUMENT_TYPE;
    PARSER_FLAG = _scLazy.PARSER_FLAG;

    const adjustAutocompleteDebounced = debounce(() => {
        $('.ui-autocomplete-input').each(function () {
            try {
                const $el = $(this);
                const widget = $el.autocomplete('instance') || $el.data('ui-autocomplete');
                if (!widget) return;
                const widgetEl = $el.autocomplete('widget');
                if (!widgetEl || !widgetEl[0]) return;
                const isOpen = widgetEl[0].style.display !== 'none';
                if (isOpen) {
