...
// ⚠️ 循环依赖修复：power_user 仅在类方法中使用（非模块级），改为惰性 Proxy 绕过循环 import。
// 此 Proxy 在模块评估期返回 undefined，运行时才惰性加载真实值，打破 TDZ 循环。
let _powerUserValue = undefined;
let _powerUserLoaded = false;
async function _loadPowerUser() {
    if (_powerUserLoaded) return;
    _powerUserLoaded = true;
    try {
        const mod = await import('../power-user.js');
        _powerUserValue = mod.power_user;
    } catch (e) {
        console.warn('[SlashCommandClosure] Failed to lazy-load power_user:', e);
    }
}
const power_user = /** @type {any} */ (new Proxy({}, {
    get(target, prop) {
        if (prop === Symbol.toStringTag) return '[object Object]';
        if (_powerUserValue !== undefined) return _powerUserValue[prop];
        return undefined;
    },
    has(target, prop) {
        return _powerUserValue !== undefined && prop in _powerUserValue;
    },
    set(target, prop, value) {
        if (_powerUserValue !== undefined) _powerUserValue[prop] = value;
        return true;
    },
    ownKeys(target) {
        return _powerUserValue !== undefined ? Reflect.ownKeys(_powerUserValue) : [];
    },
    getOwnPropertyDescriptor(target, prop) {
        return _powerUserValue !== undefined ? Reflect.getOwnPropertyDescriptor(_powerUserValue, prop) : undefined;
    },
}));
// 异步预加载，不阻塞模块评估
_loadPowerUser();
