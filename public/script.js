...
// 暴露 API 连接状态函数到全局，供 chat.html 的 API 连接器调用
if (typeof window !== 'undefined') {
    Object.defineProperty(window, 'ST_onlineStatus', {
        get: function () { return online_status; },
        set: function (v) { online_status = v; },
        configurable: true,
    });
    Object.defineProperty(window, 'ST_mainApi', {
        get: function () { return main_api; },
        set: function (v) { main_api = v; },
        configurable: true,
    });
    window.ST_setOnlineStatus = setOnlineStatus;
    window.ST_displayOnlineStatus = displayOnlineStatus;
    window.ST_startStatusLoading = startStatusLoading;
    window.ST_resultCheckStatus = resultCheckStatus;
    window.ST_getRequestHeaders = getRequestHeaders;

    // 兼容旧的访问方式
    Object.defineProperty(window, 'main_api', {
        get: function () { return main_api; },
        set: function (v) { main_api = v; },
        configurable: true,
    });
    Object.defineProperty(window, 'online_status', {
        get: function () { return online_status; },
        set: function (v) { online_status = v; },
        configurable: true,
    });
...
// 智能选择正确的 #chat：排除嵌套在 jQuery UI tabs 生成的 ui-id-* 中的 #chat
function findCorrectChatElement() {
    const allChats = document.querySelectorAll('#chat');
    if (allChats.length === 0) return null;
    if (allChats.length === 1) return allChats[0];
    
    // 存在多个 #chat，需要找到正确的那个
    // 正确的 #chat 的父链应该是：body > #sheld > #chat（不经过 #bg_tabs 或 #ui-id-*）
    // 错误的 #chat 的父链包含：#ui-id-4 或 #bg_tabs（在 #Backgrounds 内部）
    
    // 策略 1：找到不嵌套在 #Backgrounds drawer 内的 #chat
    ...
    // 策略 2：找到父元素是 #sheld（且 #sheld 是 body 的直接子元素）的 #chat
    for (const chat of allChats) {
        if (chat.parentElement && chat.parentElement.id === 'sheld') {
            if (chat.parentElement.parentElement === document.body) {
                return chat;
            }
        }
    }
    
    // 策略 3：找到包含最多 .mes 元素的 #chat
    let bestChat = allChats[0];
    let maxMes = 0;
    for (const chat of allChats) {
...
const correctChatElement = findCorrectChatElement();
let _chatElementJq = correctChatElement ? $(correctChatElement) : $('#chat');

// chatElement 使用 Proxy 动态解析正确的 #chat 元素
// 这解决了 jQuery UI tabs 在初始化后可能改变 DOM 结构的问题
export const chatElement = new Proxy(_chatElementJq, {
    get(target, prop) {
        // 每次访问 chatElement 时，重新检查是否有正确的 #chat 元素
        const el = findCorrectChatElement();
        if (el && el !== _chatElementJq[0]) {
            _chatElementJq = $(el);
        }
        const value = _chatElementJq[prop];
        // 如果是函数，绑定到当前的 jQuery 对象
        if (typeof value === 'function') {
            return value.bind(_chatElementJq);
        }
        return value;
    },
    set(target, prop, value) {
        _chatElementJq[prop] = value;
        return true;
    }
});

// 重新初始化 chatElement 的辅助函数（供需要时调用）
export function reinitChatElement() {
    const el = findCorrectChatElement();
    if (el) {
        _chatElementJq = $(el);
    }
...
export function displayOnlineStatus() {
    if (online_status == 'no_connection') {
        ...
        // Update send_textarea placeholder
        const sendTextarea = document.getElementById('send_textarea');
        if (sendTextarea) {
            const noConnectionText = sendTextarea.getAttribute('no_connection_text') || 'Not connected to API!';
            sendTextarea.placeholder = noConnectionText;
        }
        // Add no-connection class to send_form
        const sendForm = document.getElementById('send_form');
        if (sendForm) {
            sendForm.classList.add('no-connection');
        }
    } else {
        ...
        const sendTextarea = document.getElementById('send_textarea');
        if (sendTextarea) {
            const connectedText = sendTextarea.getAttribute('connected_text') || 'Type a message, or /? for help';
            sendTextarea.placeholder = connectedText;
        }
        // Remove no-connection class from send_form
        const sendForm = document.getElementById('send_form');
        if (sendForm) {
...
export async function selectCharacterById(id, { switchMenu = true } = {}) {
    ...
    if (selected_group || String(this_chid) !== String(id)) {
        ...
        console.warn('selectCharacterById error (safe to ignore):', err?.message || err);
    }
}
...
// Helper: ensure character text fields are strings (prevents .trim() errors in extensions)
// Converts all text fields (including nested char.data fields) to string type
function normalizeCharacterFields(charData) {
    ...
    const safeString = (v) => {
        ...
        if (Array.isArray(v)) {
            // Some character cards provide these as arrays of duplicates
            const nonEmpty = v.filter(x => x !== null && x !== undefined);
            if (nonEmpty.length === 0) return '';
            return typeof nonEmpty[0] === 'string' ? nonEmpty.join('') : String(nonEmpty[0]);
        }
        return String(v);
    };

    // Top-level text fields - ensure they are string type
    const textFields = ['description', 'personality', 'scenario', 'mes_example', 'first_mes', 'creatorcomment'];
    textFields.forEach(field => {
        charData[field] = safeString(charData[field]);
    });

    // char.data nested fields - also ensure these are string type
    if (charData.data) {
        const dataTextFields = ['system_prompt', 'post_history_instructions', 'creator_notes', 'mes_example', 'description', 'scenario', 'personality', 'first_mes'];
        dataTextFields.forEach(field => {
            if (field in charData.data) {
                charData.data[field] = safeString(charData.data[field]);
            } else {
                charData.data[field] = '';
            }
        });
        ...
        if (Array.isArray(charData.data.alternate_greetings)) {
            charData.data.alternate_greetings = charData.data.alternate_greetings.map(g => safeString(g));
        }
    }
...
export async function getOneCharacter(avatarUrl) {
    const response = await apiFetch('/api/characters/get', {
        method: 'POST',
        body: JSON.stringify({
    ...
    if (response.ok) {
        ...
        normalizeCharacterFields(getData);
...
export async function getCharacters() {
    const response = await apiFetch('/api/characters/all', {
        method: 'POST',
        body: JSON.stringify({}),
    });
    if (response.ok) {
        ...
        for (let i = 0; i < getData.length; i++) {
            // Normalize character fields BEFORE adding to the characters array
            // This ensures that any code (including extensions) accessing characters[i]
            // will always see string values for text fields
            const charData = getData[i];
            normalizeCharacterFields(charData);
            charData.name = DOMPurify.sanitize(charData.name);
            characters[i] = charData;
...
export async function deleteCharacterChatByName(characterId, fileName) {
    ...
    if (fileName === character.chat) {
        const chatsResponse = await apiFetch('/api/characters/chats', {
            method: 'POST',
...
export function addOneMessage(mes, { type = undefined, insertAfter = null, scroll = true, insertBefore = null, forceId = null, showSwipes = true } = {}) {
    try {
        if (!mes) {
            console.warn('addOneMessage: message is null or undefined');
            return null;
        }
...
export function updateMessageElement(mes, { messageId = chat.length - 1, messageElement = messageTemplate.clone(), adjustMediaScroll = SCROLL_BEHAVIOR.NONE } = {}) {
    try {
        ...
        console.error('updateMessageElement error:', err);
        return messageElement;
    }
}
...
export function getCharacterCardFieldsLazy({ chid = undefined } = {}) {
    ...
    const resolvers = {
        persona: () => baseChatReplace(typeof power_user.persona_description === 'string' ? power_user.persona_description.trim() : String(power_user.persona_description ?? '')),
        system: () => {
            if (!character) return '';
            const rawSystemPrompt = chat_metadata.system_prompt || character.data?.system_prompt || '';
            const systemPrompt = typeof rawSystemPrompt === 'string' ? rawSystemPrompt : String(rawSystemPrompt ?? '');
            return power_user.prefer_character_prompt ? baseChatReplace(systemPrompt.trim()) : '';
        },
        jailbreak: () => {
            if (!character) return '';
            const raw = character.data?.post_history_instructions;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return power_user.prefer_character_jailbreak ? baseChatReplace(text.trim()) : '';
        },
        version: () => character?.data?.character_version ?? '',
        charDepthPrompt: () => {
            if (!character) return '';
            const raw = character.data?.extensions?.depth_prompt?.prompt;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return baseChatReplace(text.trim());
        },
        creatorNotes: () => {
            if (!character) return '';
            const raw = character.data?.creator_notes;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return baseChatReplace(text.trim());
        },
        ...
        personality: () => {
            ...
            if (!character) return '';
            const raw = character.personality;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return baseChatReplace(text.trim());
        },
        scenario: () => {
            ...
            if (!character) return '';
            const raw = chat_metadata.scenario || character.scenario;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return baseChatReplace(text.trim());
        },
        mesExamples: () => {
            ...
            if (!character) return '';
            const raw = chat_metadata.mes_example || character.mes_example;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return baseChatReplace(text.trim());
        },
        firstMessage: () => {
            if (!character) return '';
            const raw = character.first_mes;
            const text = typeof raw === 'string' ? raw : String(raw ?? '');
            return baseChatReplace(text.trim());
        },
        alternateGreetings: () => {
            ...
            return altGreetings.map(greeting => {
                const text = typeof greeting === 'string' ? greeting : String(greeting ?? '');
                return baseChatReplace(text.trim());
            });
        },
    };
...
export async function duplicateCharacter({ avatar = null, silent = false } = {}) {
    ...
    const response = await apiFetch('/api/characters/duplicate', {
        method: 'POST',
...
export async function renameCharacter(name = null, { silent = false, renameChats = null } = {}) {
    ...
    const response = await apiFetch('/api/characters/rename', {
        method: 'POST',
        body,
    });
...
async function renamePastChats(oldAvatar, newAvatar, newName) {
    ...
    for (const { file_name } of pastChats) {
        try {
            ...
            const getChatResponse = await apiFetch('/api/chats/get', {
                method: 'POST',
                cache: 'no-cache',
                body: JSON.stringify({
                    ch_name: newName,
...
export async function getChat() {
    try {
        ...
        const response = await apiFetch('/api/chats/get', {
            method: 'POST',
            cache: 'no-cache',
            body: JSON.stringify({
                ch_name: characters[this_chid].name,
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            // Normalize chat_metadata text fields to string type to prevent .trim() errors in extensions
            const safeMetadataString = (v) => {
                if (typeof v === 'string') return v;
                if (v === null || v === undefined) return '';
                if (Array.isArray(v)) {
                    const nonEmpty = v.filter(x => x !== null && x !== undefined);
            ...
            const metadataTextFields = ['system_prompt', 'mes_example', 'scenario', 'first_mes', 'description', 'personality', 'post_history_instructions', 'creator_notes'];
            metadataTextFields.forEach(field => {
                if (field in chat_metadata) {
                    chat_metadata[field] = safeMetadataString(chat_metadata[field]);
                }
            });
...
export async function getSettings(initLoaderHandle = null) {
    ...
    if (data.result != 'file not find' && data.settings) {
        ...
        // chat.html 页面通过 window.__DISABLE_EXTENSIONS__ 跳过扩展加载
        if (data.enable_extensions && !window.__DISABLE_EXTENSIONS__) {
...
export async function saveSettings(loopCounter = 0) {
    ...
    // chat.html：API endpoint may not exist, skip save to avoid network errors
    if (window.__DISABLE_EXTENSIONS__) {
        return;
    }
...
export async function getPastCharacterChats(characterId = null) {
    ...
    const response = await apiFetch('/api/characters/chats', {
        method: 'POST',
...
export function selectRightMenuWithAnimation(selectedMenuId) {
    ...
    document.querySelectorAll('#right-nav-panel .right_menu').forEach((menu) => {
        ...
        console.warn('selectRightMenuWithAnimation error (safe to ignore):', err?.message || err);
    }
}
...
export function select_selected_character(chid, { switchMenu = true } = {}) {
    ...
    $('#system_prompt_textarea').val(typeof characters[chid].data?.system_prompt === 'string' ? characters[chid].data.system_prompt : String(characters[chid].data?.system_prompt ?? ''));
    ...
    saveSettingsDebounced();
        ...
        console.warn('select_selected_character error (safe to ignore):', err?.message || err);
    }
}
...
function select_rm_create({ switchMenu = true } = {}) {
    ...
    $('#character_open_media_overrides').hide();
        ...
        console.warn('select_rm_create error (safe to ignore):', err?.message || err);
    }
}
...
export async function setCharacterSettingsOverrides() {
    ...
    const scenarioOverrideValue = typeof chat_metadata.scenario === 'string' ? chat_metadata.scenario : String(chat_metadata.scenario ?? '');
    const exampleMessagesValue = typeof chat_metadata.mes_example === 'string' ? chat_metadata.mes_example : String(chat_metadata.mes_example ?? '');
    const systemPromptValue = typeof chat_metadata.system_prompt === 'string' ? chat_metadata.system_prompt : String(chat_metadata.system_prompt ?? '');
